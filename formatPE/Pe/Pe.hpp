...
enum class ImgType : unsigned char { file, module };

using Rva     = unsigned int;
using Ordinal = unsigned short;

enum class ImportType { unknown, name, ordinal };

enum class ExportType { unknown, exact, forwarder };

enum class RelocType { unknown, absolute, high, low, highlow, highadj, dir64 };
...
struct Reloc {
    ...
    RelocType type() const noexcept {
        ...
        case IMAGE_REL_BASED_ABSOLUTE:
            return RelocType::absolute;
        case IMAGE_REL_BASED_HIGH:
            return RelocType::high;
        case IMAGE_REL_BASED_LOW:
            return RelocType::low;
        case IMAGE_REL_BASED_HIGHLOW:
            return RelocType::highlow;
        case IMAGE_REL_BASED_HIGHADJ:
            return RelocType::highadj;
        case IMAGE_REL_BASED_DIR64:
            return RelocType::dir64;
        default:
...
struct GenericTypes {
    ...
    struct RelocsTable {
        ...
        unsigned int count() const noexcept { return hdr.relocsSizeInBytes / sizeof(Reloc); }
    };
...
struct Types<Arch::x64> : public GenericTypes {
    ...
    static_assert(sizeof(ImportAddressTableEntry) == sizeof(unsigned long long),
                  "Invalid size of ImportAddressTableEntry");
    using ImportLookupTableEntry = ImportAddressTableEntry;
    using ImportNameTableEntry   = ImportAddressTableEntry;
...
public:
    ...
    const DosHeader *dos() const noexcept { return static_cast<const DosHeader *>(m_base); }
    ...
    const OptHeader *opt() const noexcept { return &nt()->OptionalHeader; }

    const void *mod() const noexcept { return m_base; }
...
public:
    ...
    static Pe fromFile(const void *const base) noexcept { return Pe(ImgType::file, base); }

    static Pe fromModule(const void *const base) noexcept { return Pe(ImgType::module, base); }

    PeHeaders<arch> headers() const noexcept { return PeHeaders<arch>(m_base); }
    ...
    const Type *byRva(const Rva rva) const noexcept {
        ...
        for (const auto &sec : sections()) {
            ...
            if ((rva >= sectionBase) && (rva < sectionBase + sectionSize)) {
                return reinterpret_cast<const Type *>(static_cast<const unsigned char *>(m_base) +
                                                      (sectionOffset + (rva - sectionBase)));
            }
        }
    ...
    const ImgDataDir *directory(const unsigned int id) const noexcept { return &headers().opt()->DataDirectory[id]; }
    ...
    unsigned long long imageBase() const noexcept { return headers().opt()->ImageBase; }

    unsigned long imageSize() const noexcept { return headers().opt()->SizeOfImage; }

    unsigned long long entryPoint() const noexcept {
        return static_cast<unsigned long long>(
            reinterpret_cast<size_t>(byRva<void>(headers().opt()->AddressOfEntryPoint)));
    }

    ImgType type() const noexcept { return m_type; }

    bool valid() const noexcept { return headers().valid(); }

    inline static constexpr Arch peArch() noexcept { return arch; }

    size_t poi_to_va(const void *poi) const noexcept {

        for (auto i = sections().begin(); i != sections().end(); i++) {
            if (byRva<void>(i->VirtualAddress) <= poi && poi < byRva<void>(i->VirtualAddress + i->Misc.VirtualSize)) {

                return ((char *)(poi) - (char *)(byRva<void>(i->VirtualAddress))) + i->VirtualAddress + imageBase();

                //return (reinterpret_cast<const char *>(poi) - reinterpret_cast<const char *>(m_base)) + imageBase();
            }
        }
        return 0;
    }

    void *va_to_poi(size_t va) const noexcept {
        if (this->m_type == ImgType::module) {
            return (char *)(this->m_base) + (va - imageBase());
        }

        // find the section where the virtual address is located
        for (auto i = sections().begin(); i != sections().end(); i++) {
            auto sec_va     = i->VirtualAddress + imageBase();
            auto sec_end_va = sec_va + i->Misc.VirtualSize;
            if (sec_va <= va && va < sec_end_va) {
                auto  offset_in_sec = va - sec_va;
                char *sec_poi       = i->PointerToRawData + (char *)(this->m_base);
                return reinterpret_cast<void *>(sec_poi + offset_in_sec);
            }
        }

        // the virtual address < first section virtual address
        if (va < (sections().begin()->VirtualAddress + imageBase())) {
            return (char *)(this->m_base) + (va - imageBase());
        }
...
public:
    ...
    public:
        Iterator(const Sections &owner, const unsigned int pos) noexcept : m_owner(owner), m_pos(pos) {
        ...
        bool operator==(const Iterator &it) const noexcept { return m_pos == it.m_pos; }

        bool operator!=(const Iterator &it) const noexcept { return !operator==(it); }

        const typename GenericTypes::SecHeader &operator*() const noexcept { return *operator->(); }

        const typename GenericTypes::SecHeader *operator->() const noexcept { return &m_owner.sections()[m_pos]; }
    };
...
public:
    Sections(const typename GenericTypes::SecHeader *const sections, const unsigned int count) noexcept
        : m_sections(sections), m_count(count) {}

    const typename GenericTypes::SecHeader *sections() const noexcept { return m_sections; }

    bool valid() const noexcept { return m_sections != nullptr; }

    bool empty() const noexcept { return !valid() || !m_count; }
...
public:
    ...
    bool operator==(const Iterator &it) const noexcept { return m_object == it.m_object; }

    bool operator==(TheEnd) const noexcept { return m_object == TheEnd{}; }

    bool operator!=(const Iterator &it) const noexcept { return !operator==(it); }

    bool operator!=(TheEnd) const noexcept { return !operator==(TheEnd{}); }
    ...
    const Object &operator*() const noexcept { return m_object; }

    Object &operator*() noexcept { return m_object; }
...
public:
    ...
    public:
        ...
        const ModuleEntry &lib() const noexcept { return m_lib; }

        unsigned int index() const noexcept { return m_index; }

        const typename Types<arch>::ImportAddressTableEntry *
        importAddressTableEntry() const noexcept // Import Address Table
        {
            return &m_lib.importAddressTable()[m_index];
        }

        const typename Types<arch>::ImportLookupTableEntry *
        importLookupTableEntry() const noexcept // Import Lookup Table
        {
            return &m_lib.importLookupTable()[m_index];
        }

        bool valid() const noexcept { return importLookupTableEntry()->valid(); }

        ImportType type() const noexcept { return importLookupTableEntry()->type(); }
        ...
        bool operator==(const FunctionEntry &entry) const noexcept { return index() == entry.index(); }

        bool operator==(typename Iterator<FunctionEntry>::TheEnd) const noexcept { return !valid(); }
    ...
    public:
        ModuleEntry(const Pe<arch> &pe, const typename DirImports::Type *const descriptor) noexcept
            : m_pe(pe), m_descriptor(descriptor) {}

        const Pe<arch> &pe() const noexcept { return m_pe; }

        const typename DirImports::Type *descriptor() const noexcept { return m_descriptor; }

        bool valid() const noexcept { return m_descriptor && m_descriptor->Characteristics; }

        const char *libName() const noexcept { return m_pe.byRva<char>(m_descriptor->Name); }
        ...
        bool bound() const noexcept { return descriptor()->TimeDateStamp != 0; }

        bool operator==(const ModuleEntry &entry) const noexcept { return descriptor() == entry.descriptor(); }

        bool operator==(typename Iterator<ModuleEntry>::TheEnd) const noexcept { return !valid(); }
        ...
        FunctionIterator begin() const noexcept { return FunctionIterator(*this, 0); }

        typename FunctionIterator::TheEnd end() const noexcept { return {}; }
    };
...
public:
    ...
    const typename DirImports::Type *descriptor() const noexcept { return m_pe.directory<DirImports>(); }

    bool valid() const noexcept { return descriptor() != nullptr; }
    ...
    ModuleIterator begin() const noexcept { return ModuleIterator(m_pe, descriptor()); }

    typename ModuleIterator::TheEnd end() const noexcept { return {}; }
};
...
public:
    ...
    public:
        ...
        bool valid() const noexcept { return importNameTableEntry()->valid(); }

        ImportType type() const noexcept { return importNameTableEntry()->type(); }
        ...
        unsigned long long address() const noexcept { return importAddressTableEntry()->raw; }
    ...
    public:
        ...
        const typename DirDelayedImports::Type *descriptor() const noexcept { return m_descriptor; }

        bool valid() const noexcept { return m_descriptor && m_descriptor->DllNameRVA; }

        const char *moduleName() const noexcept { return m_pe.byRva<char>(m_descriptor->DllNameRVA); }
        ...
        typename Iterator<ModuleEntry>::TheEnd end() const noexcept { return {}; }
    };
...
public:
    ...
    const typename DirDelayedImports::Type *descriptor() const noexcept { return m_pe.directory<DirDelayedImports>(); }
...
public:
    ...
    public:
        ...
        bool valid() const noexcept { return descriptor()->OffsetModuleName != 0; }

        const char *libName() const noexcept {
            return reinterpret_cast<const char *>(m_lib.directoryBase()) + descriptor()->OffsetModuleName;
        }

        unsigned int timestamp() const noexcept { return descriptor()->TimeDateStamp; }

        bool operator==(const ForwarderEntry &entry) const noexcept { return index() == entry.index(); }
    ...
    public:
        explicit ModuleEntry(const typename DirBoundImports::Type *const descriptor) noexcept
            : m_directoryBase(descriptor), m_descriptor(descriptor) {}

        const typename DirBoundImports::Type *directoryBase() const noexcept { return m_directoryBase; }

        const typename DirBoundImports::Type *descriptor() const noexcept { return m_descriptor; }

        bool valid() const noexcept { return m_descriptor && m_descriptor->OffsetModuleName; }

        bool empty() const noexcept { return !valid() || !forwardersCount(); }
        ...
        bool operator==(typename Iterator<ForwarderEntry>::TheEnd) const noexcept { return !valid(); }

        ForwarderEntry &operator++() noexcept {
            m_descriptor = reinterpret_cast<const typename DirBoundImports::Type *>(
                reinterpret_cast<const unsigned char *>(forwarders()) +
                forwardersCount() * sizeof(typename GenericTypes::BoundForwarderRef));
            return *this;
        }

        ForwarderIterator begin() const noexcept { return ForwarderIterator(*this, 0); }

        ForwarderIterator end() const noexcept { return ForwarderIterator(*this, forwardersCount()); }
    };
...
public:
    ...
    const typename DirBoundImports::Type *descriptor() const noexcept { return m_pe.directory<DirBoundImports>(); }

    bool valid() const noexcept { return descriptor() && descriptor()->OffsetModuleName; }

    ModuleIterator begin() const noexcept { return ModuleIterator(descriptor()); }
...
public:
    ...
    public:
        FunctionEntry(const Exports &exports, const unsigned int index) noexcept
            : m_exports(exports), m_exportAddressTable(exports.tables().exportAddressTable),
              m_name(exports.tables().namePointerTable), m_nameOrdinal(exports.tables().nameOrdinalTable),
              m_index(index) {}
        ...
        ExportType type() const noexcept {
            ...
            return !m_exports.contains(exportAddressTableEntry()->forwarderString) ? ExportType::exact
                                                                                   : ExportType::forwarder;
        }

        bool hasName() const noexcept { return m_index == *m_nameOrdinal; }

        const char *name() const noexcept { return hasName() ? m_exports.pe().byRva<char>(*m_name) : nullptr; }

        unsigned int ordinal() const noexcept { return m_exports.ordinalBase() + m_index; }
        ...
        bool valid() const noexcept { return m_index < m_exports.count(); }
...
public:
    ...
    public:
        ...
        Export(const void *const addressOrForwarderName, const unsigned int ordinal, const ExportType type) noexcept
            : m_ptr{addressOrForwarderName}, m_ordinal(ordinal), m_type(type) {}

        const void *address() const noexcept { return (m_type == ExportType::exact) ? m_ptr.address : nullptr; }

        const char *forwarder() const noexcept {
            return (m_type == ExportType::forwarder) ? m_ptr.forwarderName : nullptr;
        }

        unsigned int ordinal() const noexcept { return m_ordinal; }

        ExportType type() const noexcept { return m_type; }
    };
...
public:
    explicit Exports(const Pe<arch> &pe) noexcept
        : m_pe(pe), m_directory(pe.directory(DirExports::k_id)),
          m_descriptor(m_directory ? pe.byRva<typename DirExports::Type>(m_directory->VirtualAddress) : nullptr),
          m_tables(m_descriptor ? Tables{pe.byRva<typename GenericTypes::ExportAddressTableEntry>(
                                             m_descriptor->AddressOfFunctions),
    ...
    const Rva directoryRva() const noexcept { return m_directory->VirtualAddress; }

    unsigned int directorySize() const noexcept { return m_directory->Size; }

    const Tables &tables() const noexcept { return m_tables; }

    bool contains(const Rva rva) const noexcept {
        return (rva >= directoryRva()) && (rva < (directoryRva() + directorySize()));
    }

    const typename DirExports::Type *descriptor() const noexcept { return m_descriptor; }

    bool valid() const noexcept { return descriptor() != nullptr; }

    unsigned int count() const noexcept { return valid() ? descriptor()->NumberOfFunctions : 0; }
    ...
    unsigned int ordinalBase() const noexcept { return descriptor()->Base; }

    FunctionIterator begin() const noexcept { return FunctionIterator(*this, 0); }

    FunctionIterator end() const noexcept { return FunctionIterator(*this, count()); }
    ...
    Export find(const char *const funcName) const noexcept {
        ...
        const auto strByRva = [this](const Rva rva) -> const char * { return m_pe.byRva<char>(rva); };
        ...
        if (!contains(exportEntry.address)) {
            ...
            return Export(
                m_pe.byRva<void>(exportEntry.forwarderString), unbiasedOrdinal + ordinalBase(), ExportType::forwarder);
        }
    }
...
public:
    ...
    public:
        ...
        const PageEntry &page() const noexcept { return m_page; }
        ...
        bool valid() const noexcept { return m_index < m_page.count(); }

        bool operator==(const RelocEntry &entry) const noexcept { return m_index == entry.m_index; }
    ...
    public:
        PageEntry(const Relocs &relocs, const typename DirRelocs::Type *entry) noexcept
            : m_relocs(relocs), m_entry(entry) {}

        bool valid() const noexcept { return m_entry && m_entry->VirtualAddress && m_entry->SizeOfBlock; }

        const typename DirRelocs::Type *descriptor() const noexcept { return m_entry; }

        const void *page() const noexcept { return m_relocs.pe().byRva<void>(m_entry->VirtualAddress); }
        ...
        bool operator==(const PageEntry &entry) const noexcept { return descriptor() == entry.descriptor(); }

        bool operator==(typename Iterator<PageEntry>::TheEnd) const noexcept { return !valid(); }
        ...
        RelocIterator begin() const noexcept { return RelocIterator(*this, 0); }

        RelocIterator end() const noexcept { return RelocIterator(*this, count()); }
    };
...
public:
    explicit Relocs(const Pe<arch> &pe) noexcept
        : m_pe(pe), m_table(pe.directory<DirRelocs>()), m_dirSize(pe.directory(DirRelocs::k_id)->Size) {}

    const Pe<arch> &pe() const noexcept { return m_pe; }

    const typename DirRelocs::Type *relocationTable() const noexcept { return m_table; }

    bool valid() const noexcept { return relocationTable() != nullptr; }

    PageIterator begin() const noexcept { return PageIterator(*this, relocationTable()); }

    PageIterator end() const noexcept {
        return PageIterator(*this,
                            reinterpret_cast<const typename DirRelocs::Type *>(
                                reinterpret_cast<const unsigned char *>(relocationTable()) + m_dirSize));
    }
};
...
public:
    ...
    public:
        ...
        const typename DirExceptions::Type *runtimeFunction() const noexcept { return m_runtimeFunction; }

        bool valid() const noexcept { return m_runtimeFunction && m_runtimeFunction->BeginAddress; }

        bool operator==(const RuntimeFunctionEntry &entry) const noexcept {
            return runtimeFunction() == entry.runtimeFunction();
        }

        bool operator==(typename Iterator<RuntimeFunctionEntry>::TheEnd) const noexcept { return !valid(); }
...
public:
    ...
    const typename DirExceptions::Type *runtimeFunctions() const noexcept { return m_runtimeFunctions; }

    bool valid() const noexcept { return m_runtimeFunctions != nullptr; }

    RuntimeFunctionIterator begin() const noexcept { return RuntimeFunctionIterator(m_runtimeFunctions); }

    typename RuntimeFunctionIterator::TheEnd end() const noexcept { return {}; }
};
...
public:
    ...
    public:
        explicit CallbackEntry(const typename GenericTypes::FnImageTlsCallback *const callbacks)
            : m_callbackPointer(callbacks) {}

        typename GenericTypes::FnImageTlsCallback callback() const noexcept { return *m_callbackPointer; }
...
public:
    ...
    CallbackIterator begin() const noexcept { return CallbackIterator(callbacks()); }

    typename CallbackIterator::TheEnd end() const noexcept { return {}; }
};
...
//              guid.Data4[7], guid.Data4[8], age, pdbPath
//
...
public:
    ...
    public:
        ...
        const typename DirDebug::Type *debugEntry() const noexcept { return m_debugEntry; }

        bool operator==(const DebugEntry &entry) const noexcept { return m_debugEntry == entry.m_debugEntry; }
...
public:
    explicit Debug(const Pe<arch> &pe) noexcept
        : m_pe(pe), m_directory(pe.directory(DirDebug::k_id)), m_debugTable(pe.directory<DirDebug>()) {}

    const typename DirDebug::Type *debugTable() const noexcept { return m_debugTable; }

    bool valid() const noexcept { return m_debugTable != nullptr; }
    ...
    DebugIterator begin() const noexcept { return DebugEntry(debugTable()); }

    DebugIterator end() const noexcept { return DebugEntry(debugTable() + count()); }
